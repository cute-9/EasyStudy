<template>
    ...
    <div class="exam_header_btn">
        <el-button type="primary"  icon="el-icon-circle-plus" @click="newCreate">新建</el-button>
        <el-button type="primary" icon="el-icon-folder-opened" @click="dataManagement">资料库</el-button>
    </div>
    <hr/>
    <div v-if="records.length==0" class="noclass">暂无此课程</div>
    <div class='exam_detail'>
      <div v-for="(item , index) in examdetail" :key="index"> 
     <exam-detail :examdetail="item" :key="item.id" @deletecessss="deletecessss" ></exam-detail>      
  </div>
    </div>
    <!-- 新建 -->
    <el-dialog
...
<div style="margin-bottom:20px">
  <el-radio v-model="radio" label="1">手动组卷(重新创建卷子）</el-radio></div>
